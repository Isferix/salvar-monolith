...
.PHONY: compile secrets up down rebuild logs ps test-deploy server-up web-up
...
server-up:
	$(call run_with_agent, devcontainer up --workspace-folder $(SERVER_WORKSPACE))
...
down:
	devcontainer down --workspace-folder $(SERVER_WORKSPACE)
# 	devcontainer down --workspace-folder $(WEB_WORKSPACE)

rebuild: compile
	devcontainer build --workspace-folder $(SERVER_WORKSPACE) --no-cache
